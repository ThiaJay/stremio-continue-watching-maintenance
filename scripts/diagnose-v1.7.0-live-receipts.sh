...
fs.writeFileSync("v170-live-receipt.json",JSON.stringify(safe,null,2)+"\n");
for(const [name,v] of Object.entries({bb:safe.breakingBad,ni:safe.northernIreland})){
  fs.writeFileSync("v170-"+name+".txt","v170_"+name+"_b"+v.count+"_latest"+v.latest+"\n");
}
const m=safe.maintenance;
fs.writeFileSync("v170-maintenance.txt","v170_run_b"+m.batch+"of"+m.batches+"_c"+m.candidates+"_a"+m.attempted+"_v"+m.verified+"_s"+m.stopped+(m.errors.length?"_e"+m.errors.join("-"):"") + "\n");
