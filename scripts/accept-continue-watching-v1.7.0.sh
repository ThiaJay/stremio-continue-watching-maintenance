...
for attempt in $(seq 1 30); do
  ...
  case "$active_normalized" in
    ...
    *)
      ...
      printf 'deployment_mismatch_%s\n' "$current_prefix" > cw-v170-acceptance-stage.txt
...
fs.writeFileSync("cw-v170-acceptance.txt",context+"\n");
fs.writeFileSync("cw-v170-acceptance.json",JSON.stringify({
...
NODE
    printf "passed\n" > cw-v170-acceptance-stage.txt
    echo "v1.7.0 exact-item acceptance passed"
    exit 0
  fi

  printf "waiting_%s\n" "$attempt" > cw-v170-acceptance-stage.txt
  echo "target not yet repaired attempt $attempt"
  sleep 30
done

echo "::error::No verified v1.7.0 target cleanup within bounded acceptance window"
exit 1
