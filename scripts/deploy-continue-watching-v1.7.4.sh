...
http_code="$(curl -sS -o deploy-response.json -w '%{http_code}' -X PUT   "${auth[@]}"   -F 'metadata=@metadata.json;type=application/json'   -F 'worker.js=@release-worker.js;filename=worker.js;type=application/javascript+module'   "$base/content")"
