...
for attempt in $(seq 1 30); do
  ...
  case "$active_normalized" in
    ...
    *)
      ...
      printf 'deployment_mismatch_%s\n' "$current_prefix" > cw-v168-acceptance-stage.txt
      echo "::error::v1.6.8 expected deployment prefix does not match current deployment"
      exit 2
      ;;
  esac
...
fs.writeFileSync("cw-v168-acceptance.txt",context+"\n");
fs.writeFileSync("cw-v168-acceptance.json",JSON.stringify({
...
NODE
    printf "passed\n" > cw-v168-acceptance-stage.txt
    echo "v1.6.8 exact-item acceptance passed"
    exit 0
  fi

  printf "waiting_%s\n" "$attempt" > cw-v168-acceptance-stage.txt
  echo "target not yet repaired attempt $attempt"
  sleep 30
done

echo "::error::No verified v1.6.8 target cleanup within bounded acceptance window"
exit 1
