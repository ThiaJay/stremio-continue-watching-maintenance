...
fs.writeFileSync("cw-v167-acceptance.txt",context+"\n");
fs.writeFileSync("cw-v167-acceptance.json",JSON.stringify({
...
NODE
    printf "passed\n" > cw-v167-acceptance-stage.txt
    echo "v1.6.7 exact-item acceptance passed"
    exit 0
  fi

  printf "waiting_%s\n" "$attempt" > cw-v167-acceptance-stage.txt
  echo "target not yet repaired attempt $attempt"
  sleep 30
done

echo "::error::No verified v1.6.7 target cleanup within bounded acceptance window"
exit 1
