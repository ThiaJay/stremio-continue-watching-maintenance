...
EXPECTED_SOURCE_SHA256="9be4a03f457f3d5217092e694a32499d245a399d7d39b2bea3c5f0b9ed9fbe34"
...
headers=Path("post-headers.txt").read_bytes()
body=Path("post-content.bin").read_bytes()
matches=re.findall(br"(?im)^content-type:\s*([^\r\n]+)",headers)
ctype=matches[-1].decode("latin1").strip() if matches else ""
found=None
if ctype.lower().startswith("multipart/"):
    ...
    found=body
if found is None:
    raise SystemExit("deployed module missing")
digest=hashlib.sha256(found).hexdigest()
...
Path("ops-receipts/wrapper-manifest-refresh-deployment-20260923.json").write_text(json.dumps({
  "schema":1,
  "result":"deployed",
  "sourceSha256":digest,
  "settingsPreserved":True,
  "testsPassed":True,
  "refreshPolicy":"same installed wrapper transport, stable wrapper id, required resources, non-generic visible name, encrypted backup and exact collection readback"
},indent=2)+"\n")
